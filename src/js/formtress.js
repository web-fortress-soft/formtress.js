(function() {
    ...
    const Formtress = (() => {
        ...
        // Function to publish error events
        const publishError = (message, error) => {
            const event = new CustomEvent('formtress:error', {
                detail: { message, error }
            });
            document.dispatchEvent(event);
        };
        const publishInfo = (message) => {
            const event = new CustomEvent('formtress:info', {
                detail: { message }
            });
            document.dispatchEvent(event);
        };
        const publishWarning = (message) => {
            const event = new CustomEvent('formtress:warning', {
                detail: { message }
            });
            document.dispatchEvent(event);
        };
        ...
        // Function to auto-discover and apply configuration from global window object
        const autoDiscoverConfig = () => {
            if (window.FormtressConfig) {
                if (!Object.isFrozen(window.FormtressConfig)) {
                    console.error('Formtress: Configuration is not frozen. Potential tampering detected. Configuration will not be applied.');
                    return;
                }
                try {
                    const config = window.FormtressConfig;
                    // Validate and apply the configuration
                    validateConfig(config);
                    applyConfigToAllForms(config);
                    console.log('Formtress: Configuration loaded from global window object.');
                } catch (error) {
                    console.error('Formtress: Failed to load configuration from global window object.', error);
                }
            }
        };
        // Function to apply configuration to all forms
        const applyConfigToAllForms = (config) => {
            const forms = document.querySelectorAll('form');
            forms.forEach(form => {
                try {
                    applyConfig(form, config);
                } catch (error) {
                    console.error(`Failed to apply config to form:`, form, error);
                }
            });
        };
        ...
        const validateConfig = (config, schema = CONFIG_SCHEMA, path = []) => {
            ...
            for (const [key, value] of Object.entries(config)) {
                ...
                if (!schema[key]) {
                    publishWarning(`Unknown configuration key: ${currentPath.join('.')}`);
                    continue;
                }
        ...
        const applyConfig = (form, config) => {
            ...
            try {
                ...
                publishError('Failed to apply configuration:', error);
                throw error;
            }
        };
        ...
        const loadConfigFromJson = async (url) => {
            try {
                ...
                publishError('Failed to load configuration:', error);
                throw error;
            }
        };
        ...
        const deepMerge = (target, source) => {
            ...
            Object.keys(source).forEach(key => {
                ...
                if (!isSafeKey(key)) {
                    ...
                    publishWarning(`Attempted prototype pollution detected with key: ${key}`);
                    return;
                }
        ...
        /**
            ...
            validateCsrf(form) {
                if (!this.csrfEnabled) {
                    ...
                    publishWarning('Formtress: CSRF protection is not enabled. Call enableCsrf() to enable it.');
                    return true;
                }
    ...
    class FormtressObserver {
        ...
        secureForm(form) {
            try {
                ...
                publishError('Formtress: Failed to secure form', error);
            }
        }
    }
    ...
    // Auto-discover configuration from global window object
    autoDiscoverConfig();

    publishInfo('Formtress: Initialized through DOMContentLoaded');
    // Public API
    return Object.freeze({
        ...
        inject: (target, config) => {
            ...
            const results = Array.from(forms).map(form => {
                try {
                    ...
                    publishError(`Failed to inject config for form:`, form, error);
                    return false;
                }
            });
        ...
        injectFromUrl: async (target, url) => {
            try {
                ...
                return SecureFormtressConfigInjector .inject(target, config);
            } catch (error) {
                //console.error('Failed to inject configuration from URL:', error);
                publishError('Failed to inject configuration from URL:', error);
                throw error;
            }
        },
