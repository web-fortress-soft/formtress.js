...
(function() {
    ...
    const isDevelopment = /^(localhost|127\.0\.0\.1|192\.168\.)|(:[0-9]{4})/i.test(window.location.host);
    ...
    const Formtress = (() => {
        ...
        class SecurityCore {
            ...
            updateCsrfField(token) {
                const forms = document.querySelectorAll(`form[${randomFormFlagId}]`);
                forms.forEach(form => {
         ...
         */
        const SecurityUtils = {
            // Generic timing check with callback
            createTimingCheck: (threshold = 100, callback) => {
                return () => {
                    const start = performance.now();
                    if(!isDevelopment) {
                        debugger;
            ...
            // Generic stack trace analyzer
            createStackAnalyzer: (patterns, callback) => {
                return () => {
                    try {
                        throw new Error();
                    } catch(e) {
                        const stack = e.stack;
                        const detected = patterns.some(pattern => stack.includes(pattern));
                        if (detected) {
                            callback?.();
                            return true;
                        }
            ...
            // Generic function protection
            protectFunction: (obj, funcName, replacement = null) => {
            ...
            // Generic interval runner with random timing
            createRandomRunner: (callback, minInterval = 500, maxInterval = 2000) => {
                const run = () => {
                    const nextRun = minInterval + Math.random() * (maxInterval - minInterval);
                    setTimeout(() => {
                        callback();
                        run();
                    }, nextRun);
            ...
            createFeatureDetector: (features, callback) => {
                return () => {
                    const detected = features.some(feature => {
                        try {
                            return feature();
                        } catch {
                            return false;
        ...
        // Security feature factory
        const SecurityFactory = {
            // Create debugger detection
            createDebuggerDetection: (options = {}) => {
                const {
                    timing = 100,
                    maxHits = 3,
                    onDetected = null,
                ...
                const check = SecurityUtils.createTimingCheck(timing, () => {
                    hits++;
                    if (hits >= maxHits) {
                        onDetected?.();
                    }
                ...
                if (checkInterval) {
                    SecurityUtils.createRandomRunner(check, checkInterval, checkInterval * 2);
                }

                return check;
            },

            // Create dev tools detection
            createDevToolsDetection: (options = {}) => {
                const {
                    onDetected = null,
                    patterns = ['debugger', 'DevTools', 'console'],
                    checkInterval = 1000
                } = options;

                const stackCheck = SecurityUtils.createStackAnalyzer(patterns, onDetected);
                const sizeCheck = () => window.outerHeight - window.innerHeight > 200;
                const consoleCheck = () => {
                    ...
                    return performance.now() - startTime > 20;
                };

                const check = SecurityUtils.createFeatureDetector([
                    stackCheck,
                    sizeCheck,
                    consoleCheck
                ], onDetected);
            ...
            // Create DOM protection
            createDOMProtection: (options = {}) => {
                const {
                    onTamper = null,
                    protectedElements = ['script', 'iframe'],
                    attributes = ['src', 'href', 'integrity']
                } = options;

                return new MutationObserver((mutations) => {
                    mutations.forEach(mutation => {
                        mutation.addedNodes.forEach(node => {
                            if (node.nodeType === 1 && protectedElements.includes(node.tagName.toLowerCase())) {
                                onTamper?.();
                                node.remove();
                            }
                        });
        ...
        // Security Layer Creator
        const createSecurityLayer = (options = {}) => {
            const {
                debug = true,
                devTools = true,
                dom = true,
                onDetection = () => window.location.reload(),
                checkInterval = 1000
            } = options;
            ...
            if (debug) {
                layers.push(
                    SecurityFactory.createDebuggerDetection({
                        onDetected: onDetection,
                        checkInterval
                    })
                );
            }
            ...
            if (devTools) {
                layers.push(
                    SecurityFactory.createDevToolsDetection({
                        onDetected: onDetection,
                        checkInterval
                    })
                );
            ...
            if (dom) {
                const observer = SecurityFactory.createDOMProtection({
                    onTamper: onDetection
                });
                ...
                layers.push(() => observer.takeRecords().length > 0);
            }

            // Return composite check function
        ...
        /**
         * Run security checks in random intervals
         */
        SecurityUtils.createRandomRunner(security);

        /** 
         * Protect common timing and security functions
         */
        ['setTimeout', 'setInterval', 'Date', 'performance'].forEach(func => {
            SecurityUtils.protectFunction(window, func);
        });
        ...
        class FormtressForm {
            ...
            constructor(form, customConfig = {}) {
                ...
                setTimeout(() => {
                    ...
                    try {
                        ...
                        clearTimeout(initTimeout);
                        console.error('Formtress: Configuration error detected. This is a critical error and the page will keep reloading until the error is fixed.',error);
                        if(!isDevelopment) {
                            window.location.reload();
                        }
                    }
            ...
            initializeForm() {
                ...
                form.setAttribute(randomFormFlagId, 'true');
            ...
            initializeField(field) {
                if (field.type === 'submit' || field.type === 'reset' || field.getAttribute(randomFormFlagId)) {
                    return;
                }
                ...
                // Mark as secured
                field.setAttribute(randomFormFlagId, 'true');
                    ...
                    mutations.forEach((mutation) => {
                        mutation.addedNodes.forEach((node) => {
                            if (node instanceof HTMLElement) {
                                if (node.nodeName === 'FORM' && !node.getAttribute(randomFormFlagId)) {
